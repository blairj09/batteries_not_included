...
## TL;DR
`testthat` provides a convenient and easy to use unit test framework for R. While traditionally used as a formal part of [package development](http://r-pkgs.had.co.nz/tests.html), it can also be used interactively. Ad hoc testing suites can be run as functions within an R session to quickly test the impact of code changes. I use this workflow when writing parsing functions for HTML data.

## Introduction
Like all Hadley Wickham creations, `testthat` is a wonderful tool that generally improves the lives of R users. As stated upfront in the package documentation, `testthat` was designed to make testing both easy and fun for R users. As a unit test framework, it should come as no surprise that the main use case for `testthat` is unit testing for R packages. The typical unit testing workflow is described in detail [here](http://r-pkgs.had.co.nz/tests.html).
...
In this case even though there are code failures the chunk runs to completion. It possible to identify where the errors occured and what specifically went wrong in a nice format. However, one remaining problem is that there's no way to differentiate in the test results what version of the function is being tested since the generic `add_fun()` from `test_add()` is all that's reported. In this case it's not a big deal because there are only two functions being tested and since we test them in succession it's easy to identify which test corresponds with which function. However, if we were testing a larger collection of functions this would be a hastle. Luckily, using `testthat::context()` and some `rlang` magic, we can provide insight into which function is associated with each set of test results.
...
And just like that we have ad hoc informative testing that identifies tests that failed along with the function that was being tested at the time of failure. To be clear, I'm not suggesting this replace unit testing in any way. I've simply found this workflow useful when I'm evaluating the effectiveness of different functions on specific tasks with a high volume of edge cases (primarily parsing html).

## Passing Additional Information
There are times when it's helpful to pass additional information to the test results so that messages are more informative. For example, in my post about [scraping Friends data]() I used `testthat` to check that certain expectations were met for each episode I parsed. When a test failed, it was helpful to know which particular episode the test failed on so that I could investigate the problem further. This can be accomplished by providing the `info` argument to any `expect_...` function. This can be especially powerful when testing a function over several cases (like several episodes). What's nice is that tests can be run over a collection of data using `purrr::map()` or `purr::map2()`.
...
While this is once again a clearly contrived case, hoepfully it is clear to see how this type of convention could be useful in a more realistic setting - particularly when additional details about the failed test are required. In this case, we're able to see the `x` and `y` value of each observation that failed the test.